...
function rime.sciomancer.attack()
	...
	if rime.sciomancer.needs.repay then
...
rime.sciomancer.attacks = {
	...
	["firefly"] = {
		"cast blast target",
		choice = function()
			return rime.sciomancer.attacks.firefly[1], "eq", "firefly"
		end,
		can = function()
			local target = rime.target
			if rime.pvp.has_def("firefly", target) then
				return true
			else
				return false
			end
	...
	["shaderot_benign"] = {
		...
		choice = function()
			...
			if not rime.pvp.has_aff("sphere", target) and (rime.sciomancer.debt_allowance > 0 or rime.sciomancer.can_repay()) then
				return rime.sciomancer.attacks.shaderot_benign[2], "eq", "shaderot_benign"
			else
				return rime.sciomancer.attacks.shaderot_benign[1], "eq", "shaderot_benign"
			end
		end,
		can = function()
		local target = rime.target
			if not rime.pvp.has_aff("shaderot_benign", target) then return true else return false end
		end,		
		debt = function()
			if string.find(rime.sciomancer.attacks.shaderot_benign.choice(), "shadowprice") then
				return 1
			else
				return 0
			end
