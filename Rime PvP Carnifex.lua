rime.pvp.Carnifex.routes = {
    ...
    ["hammer"] = {
        ...
        ["hounds"] = {"blurry_vision", "frozen", "ablaze", "bite",},
    },
    ...
    ["bhammer"] = {
        ...
        ["deathlore"] = {"glasslimb", "distortion", "wraith"},
        ["hounds"] = {"blurry_vision", "frozen", "ablaze", "bite",},
    },
...
carn.slashes = {
    ["implantProc"] = {
        ...
        if #rime.missingAff("asthma/implant", "/") == 0 and (rime.pvp.has_aff("clumsiness", targ) or rime.pvp.has_aff("weariness", targ)) then
            return true
        end
        return false
    end,
        ...
        if not rime.pvp.has_def("rebounding", targ) then
            if #rime.missingAff("asthma/slickness", "/") < 2 then
                if rime.pvp.has_aff("stupidity", targ) or rime.pvp.has_aff("impatience", targ) then
                    return true
                elseif rime.pvp.balanceCheck("focus", targ) > 2.3 then
                    return true
        ...
        local targ = rime.target
         if not rime.pvp.has_def("rebounding", targ) then
            if #rime.missingAff("asthma/anorexia", "/") < 2 or ((rime.pvp.route_choice == "white" or rime.pvp.route_choice == "black") and #rime.missingAff("asthma/implant", "/") < 2 and not rime.pvp.has_aff("slickness", rime.target)) then
                if rime.pvp.has_aff("stupidity", targ) or rime.pvp.has_aff("impatience", targ) then
                    return true
                elseif rime.pvp.balanceCheck("focus", targ) > 2.4 then
                    return true
    ...
    ["buffSlickness"] = {
        ...
        if rime.pvp.has_aff("asthma", targ) and (rime.pvp.has_aff("clumsiness", targ) or rime.pvp.has_aff("weariness", targ)) then
            return true
        elseif rime.pvp.has_aff("implant", targ) and rime.pvp.has_aff("asthma", targ) then
            return true
        end
        return false
    end
    ...
    ["disfigurement"] = {
        ...
        can = function()
        local targ = rime.target
        local pet_classes = {"sentinel", "carnifex", "praenomen", "teradrim", "indorani", "warden"}
        if rime.pvp.has_aff("asthma", targ) and (rime.pvp.has_aff("clumsiness", targ) or rime.pvp.has_aff("weariness", targ)) then
...
carn.attacks = {
	...
	["shield_ally"] = {
        ...
        can = function()
            ...
            if tonumber(rime.targets[rime.pvp.ally].aggro) > 10 and not rime.targets[rime.pvp.ally].defences.shielded then
                return true
            else
                return false
            end
    ...
    ["hammerraze"] = {
        ...
        can = function()
            ...
            if carn.canSwing() and rime.targets[targ].defences.rebounding and rime.targets[targ].defences.shielded then
                return true
            elseif (rime.targets[targ].defences.shielded or rime.targets[targ].defences.rebounding) and not rime.pvp.has_aff("distortion", targ) then
                return true
            end
            return false
        end,
    ...
    ["carve"] = {
        ...
        can = function()
            local targ = rime.target
            if not rime.pvp.has_aff("soul_poison", targ) and carn.can.poison then
                return false
            elseif not rime.pvp.has_aff("soul_disease", targ) and rime.targets[targ].soul < 80 then
                return false
            elseif carn.canSwing() and rime.pvp.has_aff("haemophilia", targ) then
                return true
            end
            return false
        end,
    ...
    ["batter"] = {
        ...
        can = function()
            ...
            if rime.pvp.route_choice == "ghammer" and rime.targets[targ].parry ~= "head" then
                return true
            elseif rime.pvp.has_aff("sensitivity", targ) and not rime.pvp.canParry() and not rime.pvp.has_aff("blackout", targ) then
                return true
            elseif rime.pvp.has_aff("soul_poison", targ) and #enemyAffTable > 6 and not rime.pvp.has_aff("blackout", targ) then
                return true
            end
    ...
    ["skewer"] = {
        ...
        can = function()
            ...
            and ((rime.pvp.has_aff("sensitivity", targ) and rime.pvp.has_aff("soul_poison", target))
                or (rime.targets[targ].bloodburstCount > 2 and rime.pvp.has_aff("haemophilia", targ))) then
                return true
            end 
            return false
        end
    ...
    ["should_batter"] = {
        ...
        can = function()
            ...
            elseif rime.pvp.has_aff("blackout", rime.target) then
                return false
            elseif rime.pvp.balanceCheck("focus", rime.target) > 3 and rime.pvp.has_aff("prone", rime.target) and not rime.pvp.has_aff("indifference", rime.target) then
                return true
                ...
                if carn.canSwing() and carn.can.implant and rime.targets[targ].soul < 77 then
                    if rime.pvp.route_choice == "white" or rime.pvp.route_choice == "black" then
                        ...
                        elseif #rime.missingAff("slickness/anorexia/asthma/paresis", "/") < 2 then
                            if rime.pvp.has_aff("stupidity", targ) or rime.pvp.has_aff("impatience", targ) or rime.pvp.balanceCheck("focus", targ) > 2.4 then
                                return true
                            else
                                return false
                            end
    ...
    ["soul_poison"] = {
        ...
        can = function()
            ...
            if carn.canSwing() and rime.targets[targ].soul < 80 and #enemyAffTable > 9 and carn.can.poison then
                return true
            end
            return false
        end
        ...
        if deathlore_attack:find("implant") ~= nil then
            if rime.pvp.route_choice == "mental" then
                if #rime.missingAff("paresis/stupidity/shyness/dizziness/epilepsy/vomiting/allergies/haemophilia/sensitivity", "/") <= 2 then
                    ...
                    deathlore_attack = deathlore_attack:gsub("trigger", "kalmia")
                else
